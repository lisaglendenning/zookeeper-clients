...
import edu.uw.zookeeper.protocol.Message.ServerSession;
...
public class GetSetClientBuilder extends MeasuringClientBuilder {
     ...
     * Not thread-safe
     */
    public static final class PerfectTreePaths extends AbstractIterator<ZNodeLabel.Path> {
        ...
        @Override
        protected ZNodeLabel.Path computeNext() {
            if (finger.isEmpty()) {
                return endOfData();
            }
            ZNodeLabel.Path path = (ZNodeLabel.Path) ZNodeLabel.joined(
                    ZNodeLabel.Path.root(),
                        ZNodeLabel.joined(finger.iterator()));
            ...
            public void run() {
                ...
                SubmitIterator<Records.Request, Message.ServerResponse<?>> operations = SubmitIterator.create(
                         Iterators.transform(PerfectTreePaths.forParameters(getParameters()), 
                                 new Function<ZNodeLabel.Path, Records.Request>() {
                                    @Override
    ...
    protected Generator<? extends Records.Request> getDefaultRequestGenerator() {
        ...
        final ImmutableList.Builder<ZNodeLabel.Path> paths = ImmutableList.builder();
        paths.add(ZNodeLabel.Path.root());
    ...
    protected static class ConnectionClientExecutorServiceBuilder extends
        ...
        protected ConnectionClientExecutorServiceBuilder(
                ...
                ClientConnectionFactory<? extends ProtocolCodecConnection<ClientSession, ProtocolCodec<ClientSession, ServerSession>, Connection<ClientSession>>> clientConnectionFactory,
                ConnectionClientExecutorService<Request, ServerResponse<?>> clientExecutor,
                RuntimeModule runtime) {
